...
export const CartDrawer = () => {
  ...
  const handleCheckout = () => {
    toast.success("Order placed!", { description: "Thank you for your purchase.", position: "top-center" });
    clearCart();
    setIsOpen(false);
  };
            ...
            <>
              <div className="flex-1 overflow-y-auto pr-2 min-h-0">
                <div className="space-y-4">
                  {items.map((item) => (
                    <div key={item.variantId} className="flex gap-4 p-3 rounded-md bg-card">
              ...
              <div className="flex-shrink-0 space-y-4 pt-4 border-t">
                ...
                <Button onClick={handleCheckout} className="w-full" size="lg">
                  <ShoppingCart className="w-4 h-4 mr-2" />Checkout — ${totalPrice.toFixed(2)}
                </Button>
              </div>
            </>
          )}
